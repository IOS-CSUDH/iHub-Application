...
class TwitterAPICaller: BDBOAuth1SessionManager {
    ...
    func favoriteTweet(tweeiId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/favorites/create.json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweeiId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    ...
    func unfavoriteTweet(tweeiId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func retweet(tweeiId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "http://api.twitter.com/1.1/statuses/retweet/\(tweeiId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweeiId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
